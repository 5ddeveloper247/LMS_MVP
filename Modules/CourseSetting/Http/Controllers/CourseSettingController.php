...
class CourseSettingController extends Controller
{
    ...
    public function saveCourse(Request $request)
    {
        ...
        try {
            ...
            $course->publish = ($request->type == 9) ? 0 : 1;
            if ($request->type == 7) {
    ...
    public function courseDetails($id, $data = null)
    {
        ...
        $course_reviews = CourseReveiw::where('course_id',$id)->latest()->get()->groupBy('courseType'); 
        // dd($course_reviews);
    ...
    public function publishCourse(Request $request, $id){
        ...
        if($status == 0){
            $course->status = 0;
            $course->publish = 0;
            $template = 'Course_Unpublished';
            
        }else{
            $course->publish = 1;
            $template = 'Course_Publish_Successfully';
        }
        $course->save();
        send_email($course->user,$template,$code);
        return redirect()->back();
    }
    ...
    if($request->has('tutor_id')){
      ...
      return DataTables::of($query)
          ...
          ->addColumn('action', function (Course $course) {
              ...
              if(auth()->user()->role_id == 1){
                if($course->publish == 1){
                    $publish_html = '<a href="'.route('publishCourse',['id' => $course->id, 'status' => 0]).'" class="dropdown-item">Unpublish</a>';
                }else{
                    $publish_html = '<a href="'.route('publishCourse',['id' => $course->id, 'status' => 1]).'" class="dropdown-item">Publish</a>';
              ...
              $html = '<div class="dropdown CRM_dropdown">
                          ...
                          <a href="javascript:void(0);"  class="dropdown-item" onclick="confirm_modal(\''.route('course.delete', [$course->id]).'\')">Delete</a>'.$publish_html.'
                          </div>
                      </div>';
              return $html;
          })
