...
class Course extends Model
{
    ...
    {
        return $this->hasMany(PaymentPlans::class, 'parent_id', 'id')
        ->where('type','<>','program')
        // ->where(function ($q) {
        //     $q->where('payment_plans.type', 'full_course')->orWhere('payment_plans.type', 'prep_course_live');
        // })
        ->where('status', 1);
    }
    ...
    public function effectiveCoursePlan()
    {

        $id = $this->parent_id ?? $this->id;
        $student_count = $this->child_enrolls();
        // dd($student_count);
        return $this->coursePlans()->where(function ($q) {
            $q->where(function($q){
                $q->where('sdate', '<=', date('Y-m-d'))->where('cdate', '>', date('Y-m-d'));
        ...
        //         $q->where('payment_plans.type','full_course');
        //     }
        //     if($this->type == 6){
        //         $q->where('payment_plans.type','prep_course_live');
        //     }
        // })
        ->where('no_of_students', '>', function($subQuery) use ($student_count) {
            $subQuery->selectRaw('COUNT(*)')
                ->fromSub($student_count, 'filtered_enrollments') // Use getCourseEnrolleds() as a subquery
                ->whereColumn('filtered_enrollments.plan_id', 'payment_plans.id'); // Compare plan_id with payment_plans
        });
            // dd($result->get(),$this->type);
    }
    ...
    public function child_enrolls()
    {

        return $this->hasMany(CourseEnrolled::class, 'course_id', 'parent_id');
    }
...
//            $price= $this->offerPrice($this, $price);
    ...
    */
    ...
    public function getLoginUserTotalPercentageAttribute()
    {
        ...
        if ($this->attributes['type'] == 2) {
            ...
            if (is_null($this->attributes['type'])) {
                // $enrolled = CourseEnrolled::whereNotNull('program_id', $this->attributes['id'] ?? 0)->where('user_id', Auth::id())
                ...
                // ->where('plan_id',$enrolled->plan_id ?? null);
            } else {
    ...
    public function userTotalPercentagewrtPlan($user_id, $course_id, $enroll_id)
    {
        $enrollment = CourseEnrolled::where('id',$enroll_id)->with('plan')->first();
        if(!$enrollment){
            $percentage = 0;
            return $percentage;
        }
        $planID = ($enrollment->plan_id && $enrollment->plan_id != 0) ? $enrollment->plan_id : null;
        // $complete_lesson = LessonComplete::where('user_id', $user_id)->where('course_id', $course_id)->whereIn('plan_id',$planArr)->where('status', 1)->get();

        $complete_lessons = LessonComplete::where('user_id', $user_id)
            ->where('course_id', $course_id)
            ->where('status', 1);

        if ($planID) {
            // If $planID has a value, filter by that specific ID
            $complete_lessons->where('plan_id', $planID);
        } else {
            // If $planID is null, fetch records with plan_id = 0 or null
            $complete_lessons->where(function($query) {
                $query->where('plan_id', 0)
                    ->orWhereNull('plan_id');
            });
        }

$complete_lessons = $complete_lessons->get();

        $countCourse = count($complete_lessons);
        if ($countCourse != 0) {
            $percentage = ceil($countCourse / count($this->lessons) * 100);
        } else {
            $percentage = count($this->lessons) > 0 ? 0 : 100;
        }
        if ($percentage > 100) {
            $percentage = 100;
        }
    ...
    public function getCourseEnrolleds()
    {
         return CourseEnrolled::where('course_id', $this->parent_id)->where('course_type', $this->type);
    }
