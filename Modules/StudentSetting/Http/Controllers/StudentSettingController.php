...
class StudentSettingController extends Controller
{
    ...
    public function storeAgreementForm(Request $request)
    {
        ...
        send_email(User::find(1),'Student_Submit_Agreement_Form',[
            'url' => url('admin/student/view/'.$student->id),
    ...
    public function changeStudentFormStatus(Request $request)
    {
        ...
        $user->comment = $request->comment ?? null;
    ...
    public function search_programs(Request $request)
    {
        try {
            ...
            if ($request->ajax()) {
                $search = $request->name && $request->name != '' ?  '%'.$request->name.'%' : '%';
                $results = Program::where('programtitle','LIKE',$search)->latest()->get();
    ...
    public function search_detail($id){
                ...
                $status = $program ? 1 : 0;
                return response()->json(['status' => $status, 'data' => $program]);
    }
...
//            $program->duration = $request->duration;
    ...
    public function enrolled_students(Request $request, $program_id)
    {
        try {
            $program = Program::find($program_id);
            $students = [];
            return view('studentsetting::program_students', compact('students', 'program','request'));
        } catch (\Exception $e) {
            Toastr::error(trans('common.Operation failed'), trans('common.Failed'));
            return redirect()->back();
        }
    }
    public function studentProgramPlan(Request $request, $program_id,$student_id)
    {
        try {
            $program = Program::find($program_id);
            $allplans = StudentProgramPaymentPlans::where('program_id',$program_id)
            ->where('user_id',$student_id);
            if($request->has('plan_id')){
                $allplans = $allplans->where('plan_id',$request->plan_id);
            }
            $allplans = $allplans->orderBy('type','asc')->get();
            $students = [];
            return view('studentsetting::student_plans', compact('students', 'program','request','allplans'));
        } catch (\Exception $e) {
            Toastr::error(trans('common.Operation failed'), trans('common.Failed'));
            return redirect()->back();
        }
    ...
    public function acceptStudentProgramPlan(Request $request){
        $request->validate([
            'sdate' => 'required',
            'edate' => 'required|after:sdate'
        ]);
        $id = $request->id ?? 0;
        $plan = StudentProgramPaymentPlans::find($id);
        if($plan){
            $plan->sdate = $request->sdate;
            $plan->edate = $request->edate;
            $plan->change_request = 0;
            $plan->save();
            $student = \App\Models\User::find($plan->user_id);
            send_email($student,'accept_programplan_change_request',[
                'program' => $plan->program->programtitle,
    ...
    public function rejectStudentProgramPlan(Request $request){
        $id = $request->id ?? 0;
        $plan = StudentProgramPaymentPlans::find($id);
        if($plan){
            $plan->change_request = 0;
            $plan->save();
            $student = \App\Models\User::find($plan->user_id);
            send_email($student,'reject_programplan_change_request',[
                'program' => $plan->program->programtitle,
                'type' => 'Installment '.$plan->type,
                'sdate' => $plan->sdate,
                'edate' => $plan->edate,
    ...
    public function getStudentPaymentPlan($id)
    {
        $plan = StudentProgramPaymentPlans::find($id);
        $status = $plan ? 1 : 0;
        return response()->json(['status' => $status, 'data' => $plan]);
    }
    ...
    public function getAllProgramStudent(Request $request, $program_id)
    {
        ...
        return Datatables::of($query)
            ...
            ->addColumn('progressbar', function ($query) use ($program) {
                $courses = Course::whereIn('id', json_decode($program->allcourses) ?? [])->with('enrollUsers', 'user', 'user.courses', 'user.courses.enrollUsers', 'user.courses.lessons', 'chapters.lessons', 'enrolls', 'lessons', 'reviews', 'chapters', 'activeReviews')->orderBy('created_at', 'DESC')->get();
                $html = '';
                    ...
                    <div class='progress_percent flex-fill text-right'>
                                                    <div class='progress theme_progressBar '>
                                                        <div class='progress-bar' role='progressbar'
                                                             style='width:" . round($course->userTotalPercentagewrtPlan($query->user->id, $course->id,$query->id)) . "%'
                                                             aria-valuenow='25'
                                                             aria-valuemin='0' aria-valuemax='100'></div>
                                                    </div>
                                                    <p class='font_14 f_w_400'>" . round($course->userTotalPercentagewrtPlan($query->user->id, $course->id,$query->id)) . "% Complete</p>
                                                </div>";
                }
                return $html;
            })
            ...
            ->addColumn('status', function ($query) {
                ...
                return $view;
            })->addColumn('notify_user', function ($query) use ($program,$request) {
                $program_enroll = CourseEnrolled::where('user_id',$query->user->id)->where('program_id',$program->id)->first();
                return view('studentsetting::partials._td_action_student_programs', compact('query','program','program_enroll','request'));
                if (round($program->userTotalPercentage($query->user->id, $program->id)) < 100) {
                    $link = '<a class="" href="' . route('program.programStudentNotify', [$program->id, $query->user->id]) . '" data-id="' . $query->user->id . '" type="button">Notify</a>';
