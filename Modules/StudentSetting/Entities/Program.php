...
class Program extends Model
{
    ...
    public function effectiveProgramPlan()
    {
        $student_count = $this->totalenrolls();
        // $student_count = CourseEnrolled::where('program_id',$this->id)->where('course_type','program')->where('status',1)->count();
...
//            ->limit(1);
    ...
    public function enrollUsers()
    {
        ...
        // ->wherePivot('plan_id', $this->effectiveProgramPlan[0]->id);
    }
    ...
    public function totalEnrolls(){
        return $this->hasMany(CourseEnrolled::class, 'program_id', 'id')->where('status', 1);
    }
    ...
    public function getIsLoginUserEnrolledAttribute()
    {
        ...
        if (!$this->totalEnrolledStudent->where('id', \auth()->user()->id)->count()) {
            return false;
        } else {
            return true;
        }
