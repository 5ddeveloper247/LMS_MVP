...
class MeetingController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $class = $system_meeting->class;
            $old_date = $system_meeting->date_of_meeting;
            $old_time = $system_meeting->time_of_meeting;
          ...
          //dd($jsonData);
           ...
           $response = json_decode($response);
            ...
            foreach ($receivers as $key => $receiver) {
                ...
                send_email($receiver,'Meeting_Updated_Individual',$code);
            }
            DB::commit();
            Toastr::success('Class updated successful', 'Success');
            return redirect()->route('virtual-class.details',[$request['class_id']]);
        } catch (Exception $e) {
            GettingError($e->getMessage(), url()->current(), request()->ip(), request()->userAgent());
        }
    }
    ...
    public function cancel(Request $request){
        try {
            ...
            if(auth()->user()->role_id == 1){
                ...
                $code = [
                    'request' => 'cancelled',
                    'course' => $meet->class->course->title,
                    'stime' => date('H:i:s', strtotime($meet->start_time)),
                    'etime' => date('H:i:s', strtotime($meet->end_time)),
                    'date' => $meet->date_of_meeting,
                    'comment' => ''
                ];
                send_email($instructor,'Class_Cancelled',$code);
                send_email($admin,'Class_Cancelled',$code);
            }else{
                $update = $meeting->update(['cancelled' => 2]);
                $reason = '';
                if($request->has('comment')){
                    $updateComment = $meeting->update(['comment' => $request->comment]);
                    $reason = 'Reason: <i>'.$request->comment.'</i>';
                }
                $instructor = $meet->instructor;
                $admin = User::where('role_id',1)->first();
                $code = [
                    'request' => 'requested cancellation',
                    'course' => $meet->class->course->title,
                    'stime' => date('H:i:s', strtotime($meet->start_time)),
                    'etime' => date('H:i:s', strtotime($meet->end_time)),
                    'date' => $meet->date_of_meeting,
