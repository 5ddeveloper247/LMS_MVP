...
<script>
    @if (isset($class))
        ...
        var selectedPlanId = '{{$class->plan_id}}';
    @else
                            ...
                            <div class="col-lg-12">
                                ...
                                <div class="white-box student-details header-menu">
                                    <div class="add-visitor">
                                        ...
                                        <div class="row mt-25" id="course_plans_row">
                                            <div class="col-xl-12">
                                                <div class="primary_input">
                                                    <label class="primary_input_label d-none" id="programPlanLabel"
                                                        for="courseTypeId">{{ __('Program Plan') }}
                                                    </label>
                                                    <label class="primary_input_label" id="coursePlanLabel"
                                                        for="courseTypeId">{{ __('Course Plan') }}
                                                    </label>
                                                    <select name="plan_id" id="coursePlanId" class="primary_select select_section active mb-15 e1" required @if(isset($class)) readonly @endif><!-- assistant_instructors[] -->
...
@push('scripts')
    ...
    <script>
        ...
        // });
        $(document).on('change','.add-visitor #program_list_row select',function (e) {
            e.preventDefault();
            
            $("#coursePlanId").empty();
               	$("#coursePlanId").siblings().find('ul').empty();
               	$("#coursePlanId").siblings().find('span').empty();
                let courseId = $(this).val();
                $.ajax({

                type: 'get',
                url: '{{ URL::to('virtualclass/getcourseplans') }}',
                data: {
                    'course_id': courseId,
                    'type_id' : 'program'
                },
                ...
                success: function(data) {
                    $.each(data.data, function(i, item) {
                            $('#coursePlanId').append($('<option>', {
                                value: item.id,
                                text: item.sdate+' to '+item.edate
                            }));

                            $("#coursePlanId").siblings().find('ul').append(
                                '<li class="option" data-value="' + item.id + '">' +
                                item.sdate+' to '+item.edate+'</li>'
                          	);
                        });
        ...
        function checkProgramSelect(){
            ...
            $('#programPlanLabel').removeClass('d-none');
            $('#coursePlanLabel').addClass('d-none');
            }else{
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data.data, function(i, item) {
                            ...
                            $('#coursePlanId').niceSelect();

                    }
                });
                $('#programPlanLabel').addClass('d-none');
                $('#coursePlanLabel').removeClass('d-none');
            }
        }
