...
class VirtualClassController extends Controller
{
    ...
    public function getcourseplans(Request $request)
    {
        ...
        $child_id = ($type_id == 'program') ? array($course_id) : Course::where('parent_id',$course_id)->pluck('id')->toArray();
        // $type = json_decode($request->type_id);
        // $type_id = $type[0] ?? '0';
        switch ($type_id) {
            case '4':
                $type_val = 'full_course';
                break;
            case '6':
                    $type_val = 'prep_course_live';
                break;
            case 'program':
                    $type_val = 'program';
                # code...
            ...
            default:
                    $type_val = 'repeat_course';
                # code...
                break;
        }
        // dd($course_id,$type_val);
        $plans = DB::table('payment_plans')->whereIn('parent_id', $child_id)->where('type', $type_val)
        ->where(function($q){
            $q->where(function($q){
                $q->where('sdate','<=',date('Y-m-d')) 
    ...
    public function validateClass(Request $request)
    {
        ...
        //  $program = new Program();
          ...
          // die();
            ...
            $programplans = PaymentPlans::find($plan_id);
        }
          if(!$programplans){
            $arrRes['done'] = false;
            ...
            if($program){
                ...
                if($programconflict > 0){
                    $arrRes['done'] = false;
                    $arrRes['error'] = 'You have other Courses having same Program on given time. Please choose another Date/Time';
                    return response()->json($arrRes);
                    die();
                }
            }
        ...
        if($plan_id == ''){
          ...
          ->where('status',1)->first();
            ...
            $courseplans = PaymentPlans::find($plan_id);
        }
          // $arrRes['done'] = false;
    ...
    public function store(Request $request)
    {
        ...
        try {
          $fetchcourse = Course::where('id',$request->courses)->first();
            ...
            $class->plan_id = $request->plan_id ?? 0;
            ...
            if($request->has('plan_id')){
                $programplans = PaymentPlans::find($request->plan_id);
            }else{
              $programplans = PaymentPlans::where(function ($q) {
                  $q->where(function ($r){
                    $r->where('sdate', '<=', Carbon::today()->format('Y-m-d'))
            ...
            if($request->has('plan_id')){
                $courseplans = PaymentPlans::find($request->plan_id);
            }else{
              $courseplans = PaymentPlans::where(function ($q) {
                  $q->where(function ($r){
                    $r->where('sdate', '<=', Carbon::today()->format('Y-m-d'))
            ...
            //     }
                ...
                if ($course) {
                    ...
                    $planid = 0;
                    if(isset($programplans)){
                        $planid = $programplans->id;
                    }
                    if(isset($courseplans)){
                        $planid = $courseplans->id;
                    }
                    $query = CourseEnrolled::where('plan_id',$planid)->groupBy('user_id')->pluck('user_id')->toArray();
                    $users = User::with('sender')->where('id', '!=', Auth::id())->whereIn('id', $query)->get();


                    if (count($users)) {
                        foreach ($users as $user) {
                            send_email($user, 'Class_Created', [
                                'course' => $fetchcourse->title ?? 'Delete course',
                                'type' => $typeSlug,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $class_date = strtotime($class['start_date']);
            $today_date = strtotime(date('Y-m-d'));
            $class_startDate = ($class_date < $today_date) ? $today_date : $class_date;
            $class_start_date = ($class_date < $today_date) ? date('Y-m-d') : $class['start_date'];
            $start_date = $class_startDate;
            $end_date = strtotime($class['end_date']);
            if ($class->type == 0) {
                $end_date = strtotime($class['start_date']);
            }
            ...
            elseif ($class->host == "Team") {
                ...
                foreach ($all as $team) {
                    ...
                    // }
                    // dd($team->start_time);
                    if($team->start_time > date('Y-m-d H:s:i')){
                        // dd($team);
                ...
                if ($totalClass != 0) {
                    for (
                        ...
                        $new_date = date('m/d/Y', strtotime($class_start_date . '+' . $i . ' day'));
                        if(Carbon::parse($new_date)->is($dayofWeek)){
            ...
            $receivers = $class->parentcourse->enrollUsers;
            if ($class->type == 0) {
            ...
            foreach ($receivers as $key => $receiver) {
                $code = [
                    'course' => $class->parentcourse->title,
                    'newdate' => $class->start_date,
                    'newtime' => $class->time
                ];
                send_email($receiver,'Meeting_Updated',$code);
                $details = [
