...
class TutorsSettingController extends Controller
{
    ...
    public function setSlotTime(Request $request)
    {
        ...
        try {
            ...
            $dayofWeek = Carbon::parse($request->slot_date)->format('D');
            // dd($dayofWeek);
            ...
            $meetingConflict = VirtualClass::where('user_id',$userid)
                ...
                ->where(function ($query) use ($start_time, $end_time, $date) {
                    $stime = Carbon::parse($start_time)->format('H:i:s');
                    $etime = Carbon::parse($end_time)->format('H:i:s');
                    $query->whereBetween('time', [$stime,$etime])
    ...
    public function getAllSlots()
    {
        ...
        return Datatables::of($query)
            ...
            ->editColumn('instructor', function ($query) {
                ...
                return $query->student->name ?? '--Student Removed--';
            ...
            ->addColumn('price', function ($query) {
                ...
                if (Carbon::parse($query->assign_date) == Carbon::today()->setTimezone(Settings('active_time_zone'))) {
                    if (Carbon::parse($query->assign_start_time) <= Carbon::now()->setTimezone(Settings('active_time_zone')) && Carbon::now() <= Carbon::parse($query->assign_end_time)) {
                        $currentstat = 'started';
                    } elseif (Carbon::parse($query->assign_start_time) > Carbon::now()->setTimezone(Settings('active_time_zone'))) {
                        $currentstat = 'waiting';
                    } else {
                        $currentstat = 'closed';
                    }
                ...
                if (\Carbon\Carbon::parse($query->assign_date) > \Carbon\Carbon::now()->setTimezone(Settings('active_time_zone'))) {
                    $currentstat = 'waiting';
                }
                ...
                if ($currentstat == 'started') {
                    ...
                    ';
                } elseif($currentstat == 'closed') {
                    $html = '<a href = "#"  class="primary-btn small bg-warning border-0 text-white" > Closed</a >';
                }
                else{
                    $html = '<a href = "#"  class="primary-btn small bg-danger border-0 text-white" > Cancelled</a >';
                }
        ...
        if ($record) {
            ...
            $record->save();
             $shortCodes = [
                        ...
                        'time' => $record->slot->start_time,
                        'instructor' => $record->instructor->name,
                        'balance' => $record->price
                    ];
            send_email($user,'Slot_Cancel',$shortCodes);
