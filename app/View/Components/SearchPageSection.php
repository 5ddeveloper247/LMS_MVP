...
class SearchPageSection extends Component
{
    ...
    public function render()
    {
        ...
        $courses = Course::with('chapters','parent','parent.chapters')
        ...
        $program = Program::where('programtitle','LIKE',$search)->orderBy('seq_no', 'asc')->where('status', 1)->has('effectiveProgramPlan')->with('user','effectiveProgramPlan');
        ...
        $program_max_price = Program::where('status', 1)->has('effectiveProgramPlan')->get()->max(function($query){
            return $query->effectiveProgramPlan[0]->amount;
        });
        ...
        $courses= $courses->where(function($q) use ($max_price,$min_price){
            $q->whereHas('effectiveCoursePlan',function($q) use ($max_price,$min_price){
        ...
        $program = $program->whereHas('effectiveProgramPlan',function($q) use ($min_price,$max_price){
            $q->whereBetween('amount',[$min_price,$max_price]);
        });
