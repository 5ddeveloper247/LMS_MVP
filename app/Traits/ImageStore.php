...
trait ImageStore
{
    ...
    public static function saveFile(UploadedFile $file)
    {
        if (isset($file)) {
            $current_date = Carbon::now()->format('d-m-Y');
            if (!File::isDirectory('public/uploads/file/' . $current_date)) {
                File::makeDirectory('public/uploads/file/' . $current_date, 0777, true, true);
            }
            $fileName1 = md5(rand(0, 9999) . '_' . time()) . '.' . $file->clientExtension();
            $file_name = 'public/uploads/file/' . $current_date . '/' . $fileName1;
            $file->move(public_path('uploads/file/' . $current_date), $fileName1);
            return $file_name;
        } else {
            return null;
        }
