...
if (!function_exists('courseRating')) {
    function courseRating($course_id,$type = null)
    {
        ...
        $totalRatings['rating'] = 0;
        $ReviewList = \Modules\CourseSetting\Entities\CourseReveiw::where('course_id', $course_id)->where('courseType',$type)
            ->get();
        // $ReviewList = DB::table('courses')
        //     ->join('course_reveiws', 'course_reveiws.course_id', 'courses.id')
        //     ->select('courses.id', 'course_reveiws.id as review_id', 'course_reveiws.star as review_star')
...
if (!function_exists('getProgramListClassCount')) {
    ...
    {
        $totalCounts = [];
        $counts = Program::orderBy('seq_no', 'asc')->where('status', 1)->has('effectiveProgramPlan')->with('effectiveProgramPlan')->pluck('allcourses');
        // $counts = Program::orderBy('seq_no', 'asc')->where('status', 1)->has('currentProgramPlan')->with('currentProgramPlan')->pluck('allcourses');
        foreach ($counts as $count) {
            $totalCounts = array_merge($totalCounts, json_decode($count));
        }
        return \Modules\VirtualClass\Entities\VirtualClass::whereIn('course_id', array_unique($totalCounts))->where('course_types','LIKE', '%program%')->count();
        // return \Modules\VirtualClass\Entities\VirtualClass::whereIn('course_id', array_unique($totalCounts))->get();
    }
}
