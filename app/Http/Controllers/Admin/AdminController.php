...
class AdminController extends Controller
{
    public function enrollLogs(Request $request)
    {
        ...
        if (Auth::user()->role_id == 2) {
            ...
            $courses = Course::whereIn('type',[1,2,7,8,9])->get();
        }
    ...
    public function tutorRevenue(Request $request)
    {
        try {
            ...
            $total_tutor_revenue = $query->sum('purchase_price');
            $total_tutor_tax = $query->get()->sum(function ($enrollment) {
                return $enrollment->course->tax ?? 0; // Using null coalescence in case course or tax is null
            });
            $total_admin_revenue = $query->sum('reveune');
            $total_tutor_revenue = $total_tutor_revenue-$total_tutor_tax-$total_admin_revenue;
            $balance = User::where('id', Auth::id())->sum('balance');
        ...
        // try {
            ...
            if ($request->status == 2) {
                ...
                $codes = [
                    ...
                    'transaction' => $request->transection_id,
                    'bank_name' => $withdraw_requests->bank_name,
                    'br_code' => $withdraw_requests->branch_code,
                    'account_title' => $withdraw_requests->account_holder,
                    'account_no' => $withdraw_requests->account_number
                ];
                send_email($withdraw_requests->userRequest,'Bank_Payment',$codes);
            }
            if($request->has('decline_comment')){
                $withdraw_requests->comment = $request->decline_comment;
            }
            $comment = $request->decline_comment ?? '--No Comment--';
            ...
            return redirect()->back();
        // } catch (\Exception $e) {
        //     return response()->json(['error' => trans("lang.Oops, Something Went Wrong"), 'exception' => $e]);
        // }
    }
    ...
    public function getEnrollLogsQuiz(Request $request)
    {
        ...
        return Datatables::of($query)
            ...
            ->editColumn('type', function ($query) {
                ...
                if ($query->course_type == 2) {
                    $type = 'Big Quiz';
                }
                if ($query->course_type == 3) {
                    $type = 'Classes';
                }
                if ($query->course_type == 7) {
                    $type = 'Time Table';
                }
                if ($query->course_type == 8) {
                    $type = 'Repeat Course';
                }
                if ($query->course_type == 4) {
                    $type = 'Full Course';
                }
                if ($query->course_type == 5) {
                    $type = 'Prep-Courses<small>(On-Demand)</small>';
                }
                if ($query->course_type == 6) {
                    $type = 'Prep-Courses<small>(Live)</small>';
                }
                if ($query->course_type == 9) {
                    $type = 'Individual Course';
                }
                return $type;
            })
            ->editColumn('completion_status',function ($query){
                if($query->course->userTotalPercentage($query->user->id, $query->course->id) != 100){

                    return 'In Progress';
                }else{
                    return $query->certificate_access == 1 ? 'Certificate Generated' : 'Completed';
                }
            })
            ->editColumn('created_at', function ($query) {
                return showDate(@$query->created_at);
