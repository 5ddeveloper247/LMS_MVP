...

{
...

    {
        ...
        $otherUsers = User::whereNotIn('role_id',[1,2,3,9])->where('id','<>',Auth::user()->id)->pluck('id')->toArray();
        if (Auth::user()->role_id == 1) {
            ...
            $users = User::with('sender')->where('id', '!=', Auth::id())
            ->where(function($q) use ($query,$otherUsers){
                $q->whereIn('id',$query)
                ->orWhereIn('id',$otherUsers);
...
//            $student_ids = 	CourseEnrolled::where('course_id',null)->where('user_id' ,'!=',Auth::id())->groupBy('program_id')->groupBy('user_id')->pluck('user_id')->unique()->toArray();
            ...
            $users = User::whereIn('id', $user_ids)->orWhereIn('id',$otherUsers)->with('sender')->latest()->get();
