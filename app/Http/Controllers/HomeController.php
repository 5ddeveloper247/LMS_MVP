    ...
    public function getDashboardData()
    {
        try {
            ...
            if ($user->role_id == 2) {
                ...
                $currPackage = PackagePurchasing::where('user_id',auth()->user()->id)->where('status',1)->latest()->first();
                $course_count = ($currPackage) ? $currPackage->course_limit : 0;
                ...
                $info['totalEnroll'] = $allCourseEnrolled->count();
                $info['allCourse'] = Course::where('user_id', $user->id)->whereIn('type', [1, 2, 7, 9])->count();
                $info['activeCourse'] = Course::where('user_id', $user->id)->whereIn('type', [1, 2, 7, 9])->where('status',1)->count();
                $info['coursesLeft'] = $course_count - $info['activeCourse'];
            } else {
                ...
                $info['adminRev'] = number_format($info['adminRev'], 2, '.', '');
