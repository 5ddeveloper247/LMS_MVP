...
class StudentController extends Controller
{
    ...
    public function paymentPlanChangeRequest(Request $request){
        $id = $request->id ?? 0;
        $plan = StudentProgramPaymentPlans::find($id);
        $plan->change_reason = $request->reason ?? '';
        $plan->change_request = 1;
        $plan->save();
        $admin = User::find(1);
        $student = User::find($plan->user_id);
        $program = Program::find($plan->program_id);
        send_email($admin,'program_plan_installment_change_requested',[
            ...
            'type' => 'Installment '.$plan->type,
                ...
                'edate' => $plan->edate,
                'amount' => $plan->amount,
                'plan' => $plan->plan->sdate.' to '.$plan->plan->edate
        ]);
    ...
    public function uploadUserForm(Request $request)
    {
        ...
        $user->status = 0;
        $user->save();
        SendGeneralEmail::dispatch(User::find(1), 'Student_Submit_Agreement_Form', [
...
public function getProgramCertificate($id, $slug, Request $request)
    ...
    public function submitReview(Request $request)
    {
        ...
        try {
            ...
            $review = CourseReveiw::where('user_id', $user_id)->where('course_id', $request->course_id)->where('courseType',$request->type)->first();
            ...
            if (is_null($review)) {
                ...
                if($request->has('type')){
                    $newReview->courseType = $request->type;
                }
