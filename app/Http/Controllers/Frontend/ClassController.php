    ...
    public function classDetails($slug, Request $request)
    {
        ...
        try {
            ...
            if($request->has('program_id')){
                $isEnrolled = CourseEnrolled::where('program_id', $request->program_id)->where('plan_id',$course->class->plan_id)->where('user_id', Auth::id())->count();
                if ($isEnrolled == 0) {
                    Toastr::error('You need to buy this course to access it.', trans('common.Failed'));
                    return redirect()->back();
                }
            }else if($request->has('courseType')){
                $isEnrolled = CourseEnrolled::where('course_id', $request->course_id)->where('course_type', $request->courseType)->where('plan_id',$course->class->plan_id)->where('user_id', Auth::id())->count();
                if ($isEnrolled == 0) {
                    Toastr::error('You need to buy this course to access it.', trans('common.Failed'));
                    return redirect()->back();
                }
            ...
            // if($isEnrolled->plan_id != $course->class->plan_id){
            //     Toastr::error(trans('common.Access Denied'), trans('common.Failed'));
            //         return redirect()->back();
            // }
