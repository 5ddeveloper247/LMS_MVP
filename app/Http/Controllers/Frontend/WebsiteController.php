...
class WebsiteController extends Controller
{
    ...
    public function fullScreenView(Request $request, $course_id, $lesson_id)
    {
        ...
        try {
            ...
            if ($request->has('program_id')) {
                ...
                $enrollmentRecord = CourseEnrolled::where('program_id', $request->program_id)->where('plan_id',$plan_id)->where('user_id', $user_id)->first();
                ...
                $isEnrolled = CourseEnrolled::where('course_id', $course_id)->where('course_type', $request->courseType)->where('plan_id',$plan_id)->where('user_id', $user_id)->count();
                $enrollmentRecord = CourseEnrolled::where('course_id', $course_id)->where('course_type', $request->courseType)->where('plan_id',$plan_id)->where('user_id', $user_id)->first();
            ...
            if (isset($attempted_quiz)) {
                ...
                foreach ($all as $key => $i) {
                    ...
                    if ($check && $i->pass == 1) {
                        $lesson = LessonComplete::where('course_id', $i->course_id)->where('lesson_id', $check->id)->where('user_id', Auth::id())->where('plan_id',$plan_id)->first();
                        ...
                        if (!$lesson) {
                            ...
                            if($request->has('plan_id')){
                                $lesson->plan_id = $request->plan_id;
                            }
                        }
                        $lesson->status = 1;
                        $lesson->save();
    ...
    public function addToCartQuiz(Request $request, $id)
    {
        try {
            ...
            if (isset($course->effectiveCoursePlan[0]) && in_array(request()->get('courseType'), [4, 6])) {
                ...
                $course = Course::where('price', '!=', '0.00')
                    ->where('id', $id)
                    ->first();
            } elseif (in_array(request()->get('courseType'), [8])) {
                $course = Course::where('price', '<>', '0.00')
                    ->where('parent_id', $id)
                    ->where('type',8)
                    ->first();
            ...
            if (!Auth::check()) {
                ...
                $include_plan = $request->plan_id ? '&plan_id='.$request->plan_id : '';
                session(['redirectTo' => (!empty($course->parent_id) ? route('addToCartQuiz', [$id]) . '?courseType=' . $request->courseType.$include_plan : route('addToCartQuiz', [$id]))]);
                return \redirect()->route('login');
            }

            if(in_array(request()->get('courseType'), [4, 6]))
            {
                $enrolleds_count = CourseEnrolled::where('user_id',auth()->user()->id)->where('course_type',request()->get('courseType'))->where('course_id',$id)
                    ->whereHas('plan',function($q){
                        $q->where('edate','>=',Carbon::today());
                    })->count();
                    if($enrolleds_count > 0){
                        Toastr::error('You have already bought this Course.', 'Failed');
            ...
            if (Auth::check() && ($user->role_id != 1)) {
                ...
                if (isset($exist)) {
                    ...
                    // if (count($course->currentCoursePlan) && in_array(request()->get('courseType'), [4, 6])) {
                        $course_price = $course->effectiveCoursePlan[0]->amount;
                    ...
                    if ($request->has('courseType') || $request->courseType == 8) {
                        return redirect()->to((!empty($course->parent_id) ? route('courseDetailsView',['slug' => $course->parent->slug, 'courseType' => $request->courseType]) : route('courseDetailsView',['slug' => $course->slug, 'courseType' => $request->courseType])));
                    } else {
                        return redirect()->to((!empty($course->parent_id) ? courseDetailsUrl($course->parent_id, $course->type, $course->parent->slug) . '?courseType=' . $course->type : courseDetailsUrl($course->id, $course->type, $course->slug)));
                    }
                }
    ...
    public function buyNowQuiz(Request $request, $id)
    {
        ...
        try {
            ...
            if (count($course->effectiveCoursePlan) && in_array(request()->get('courseType'), [4, 6])) {
                // $course = Course::where('price', '==', '0.00')
                //     ->where('type', $request->courseType)
                //     ->where('parent_id', $id)
                //     ->with('currentCoursePlan')
            ...
            if (!Auth::check()) {
                Toastr::error('You must login', 'Error');
                $inlucde_plan = $request->plan_id ? '&plan_id='.$request->plan_id : '';
                session(['redirectTo' => (!empty($course->parent_id) ? route('buyNowQuiz', [$id]) . '?courseType=' . $request->courseType.$inlucde_plan : route('buyNowQuiz', [$id]))]);
                return \redirect()->route('login');
            }
    ...
    public function teachWithUs()
    {
        ...
        $packages = PackagePricing::where('status', '1')->orderBy('seq_no','desc')->take(3)->get() ;
        $about = AboutPage::first();
        ...
        try {
            ...
            if ($request->ajax()) {
                ...
                $courses = Course::orderBy('total_enrolled', 'desc')->where(function($q){
                    ...
                    ->with('user', 'parent','effectiveCoursePlan','enrolls', 'comments', 'reviews', 'lessons', 'activeReviews', 'enrollUsers', 'class', 'cartUsers', 'quiz', 'quiz.assign', 'courseLevel')->get();
                $program = Program::where('programtitle','LIKE','%'.$search.'%')->orderBy('seq_no', 'asc')->where('status', 1)->has('effectiveProgramPlan')->with('user','effectiveProgramPlan')->get();
    ...
    public function lessonComplete(Request $request)
    {
        try {
            ...
            if ($request->plan_id) {
                $lesson->where('plan_id', $request->plan_id);
            }
    ...
    public function lessonCompleteAjax(Request $request)
    {
        ...
        try {
            ...
            if ($request->has('program_id')) {
                ...
                ->count();
                $enrolled = CourseEnrolled::where('program_id', $request->program_id)->where('user_id', Auth::id())
                ->whereHas('plan',function($q){
                    $q->where(function($q){
                    $q->where('sdate','<=',now())
                    ->where('edate','>=',now());
            ...
            if ($request->has('courseType')) {
                $isEnrolled = CourseEnrolled::where('course_id', $request->course_id)->where('course_type', $request->courseType)->where('user_id', Auth::id())
                ->where(function($q){
                ...
                ->count();
                $enrolled = CourseEnrolled::where('course_id', $request->course_id)->where('course_type', $request->courseType)->where('user_id', Auth::id())
                ->where(function($q){
                    $q->whereNotIn('course_type',[4,6])
                    ->orWhere(function($q){
                        $q->whereIn('course_type',[4,6])
            ...
            if ($request->has('plan_id')) {
                $lesson = $lesson->where('plan_id',$request->get('plan_id'));
            }
            ...
            if ($course) {
                ...
                if ($request->has('plan_id')) {
                    $completeLessons = $completeLessons->where('plan_id', $request->get('plan_id'));
                }
    ...
    public function continueCourse(Request $request, $slug)
    {
        try {
            ...
            // })->first();
            if ($request->has('program_id')) {
                $enrollment = CourseEnrolled::where('program_id', $request->program_id)->where('user_id', Auth::id())
                ->whereHas('plan',function($q){
                    $q->where(function($q){
                    $q->where('sdate','<=',now())
                    ->where('edate','>=',now());
                ...
                // if($request->has('courseType'))
                $enrollment = CourseEnrolled::where('course_id', $course->id)->where('course_type', $request->courseType)->where('user_id', Auth::id())
                ->where(function($q){
                    $q->whereNotIn('course_type',[4,6])
                    ->orWhere(function($q){
                        $q->whereIn('course_type',[4,6])
            ...
            if (!empty($lesson_id)) {
                $plan_url = ($enrollmentRecord && $enrollmentRecord->plan) ? '&plan_id='.$enrollmentRecord->plan->id : '';
                if ($request->has('program_id')) {
                    return \redirect()->to(url('fullscreen-view/' . $course->id . '/' . $lesson_id) . '?program_id=' . $request->program_id.$plan_url);
                } else if ($request->has('courseType')) {
                    return \redirect()->to(url('fullscreen-view/' . $course->id . '/' . $lesson_id) . '?courseType=' . $request->courseType.$plan_url);
                }
            } else {
    ...
    public function packageBuyingCreate(Request $request)
    {
        ...
        try {
             ...
             if ($AuthorizeObj->makePayment($request, $request->type, false, null, true)) {
                ...
                if (PackagePurchasing::where('user_id', $request->user_id)->count()>0) {
                ...
                if (PackagePurchasing::where('user_id', $request->user_id)->count()>1) {
                    $last_package_title = PackagePricing::where('id', $last_package_id)->value('title') ?? '';
                    // $last_package_id = PackagePurchasing::where('user_id', $request->user_id)->latest()->value('package_id');
                    SendGeneralEmail::dispatch(\App\Models\User::find($request->user_id), 'Tutor_Package_Upgrade', [
                        ...
                        'previous_package' => $last_package_title
                    ]);
