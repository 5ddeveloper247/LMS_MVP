                        ...
                        @if (isset($programs))
                            @foreach ($programs as $SinglePrograms)
                                @php
                                    ...
                                    $planID = $SinglePrograms->plan_id ?? 0;
                                    $currentinstallment = Modules\Payment\Entities\StudentProgramPaymentPlans::where('user_id',auth()->user()->id)
                                    ...
                                    <div class="couse_wizged border">
                                        ...
                                        <div class="course_content py-3 px-2">
                                            <div class="d-flex justify-content-between my-2">
                                                <a @if($currentinstallment && $currentinstallment->pay_status == 'pay') href="javascript:void(0)" @else href="{{ route('programs.detail', [$program->id]) }}" @endif
                                                    style="width: 65%;" @if($currentinstallment && $currentinstallment->pay_status == 'pay') class="payForPlan" @endif>
                                                    <h4 class="noBrake"
                                                        title="{{ $program->programtitle }} {{ isset($SinglePrograms->plan->plan_order) ? '(Plan: '.$SinglePrograms->plan->sdate .' - '.$SinglePrograms->plan->edate. ')' : '' }}">
                                                        <!-- noBrake -->
                                                    ...
                                                    <small class="text-dark">{{ isset($SinglePrograms->plan->plan_order) ? '(Plan: '.$SinglePrograms->plan->sdate .' - '.$SinglePrograms->plan->edate. ')' : '' }}</small>
                                                </a>
                                                <div class="d-flex align-items-center gap_15">
                                                    ...
                                                    <div class="progress_percent flex-fill text-right">
                                                        @if($currentinstallment && $currentinstallment->pay_status == 'pay')
                                                            <span class="text-danger">Installment Plan Expired</span>
                                                            <a href="{{ route('my.program.payment.plan', [$program->id, 'plan_id' => isset($SinglePrograms->plan_id) ? $SinglePrograms->plan_id : '']) }}"
                                                            class="link_value theme_btn small_btn4 custom_student_btn">Pay Installment</a>
                                                        @else
                                                        <a href="{{ route('my.program.payment.plan', [$program->id, 'plan_id' => isset($SinglePrograms->plan_id) ? $SinglePrograms->plan_id : '']) }}"
                            ...
                            @if (isset($courses))
                                @foreach ($courses as $SingleCourse)
                                    @php
                                        ...
                                        // dd($course->enrolls->where('plan_id',$SingleCourse->plan_id));
                                        ...
                                        @if ($course->type == 1)
                                            <div class="quiz_wizged border w-100">
                                                ...
                                                <div class="course_content pb-2 px-2">
                                                    ...
                                                    @if($SingleCourse->plan)
                                                    <div class="d-flex justify-content-between align-items-center">
                                                        <small>{{ date(getActivePhpDateFormat(),strtotime($SingleCourse->plan->sdate)) }} to {{ date(getActivePhpDateFormat(),strtotime($SingleCourse->plan->edate)) }}</small>
                                                        @if(Carbon\Carbon::parse($SingleCourse->plan->edate) < Carbon\Carbon::today())
                                                         <span class="badge bg-warning">Plan Closed</span>
                                                        @endif
                                                    </div>
                                                    @endif
                                                    ...
                                                    <div class="course_less_students d-flex justify-content-between">
                                                        ...
                                                        @if ($course->type == 2)
                                                            <a>
                                                                ...
                                                                <i class="ti-user"></i> {{ $course->enrolls->where('plan_id',$SingleCourse->plan_id)->count() }}
                                                                {{ __('frontend.Students') }}
                                                            </a>
                                                        @else
                                                            <a>
                                                                <i class="ti-user"></i>
                                                                {{ $course->enrolls->where('plan_id',$SingleCourse->plan_id)->count() }}
                                                                {{ __('frontend.Students') }}
                                                            </a>
                                                        @endif
