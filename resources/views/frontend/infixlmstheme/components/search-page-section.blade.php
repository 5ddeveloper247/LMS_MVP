<style>
    ...
    .custom-input:focus {
        border-color: #0079a8;
        box-shadow: 0 0 5px rgba(0, 121, 168, 0.5);
    }
            ...
            <div id="filter-name-content" style="display: none">
                <div class="d-flex flex-column align-items-start px-md-5 px-2">
                    ...
                    <div class="align-items-center d-flex input-group bg-light w-75">
                        <!-- Input Field -->
                        <input type="text" class="form-control bg-transparent custom-input" id="search_query"
                            placeholder="Search for Courses / Programs"
                            value="{{$request->has('query') ? $request->get('query') : ''}}"
                            onfocus="this.placeholder = ''"
                            onblur="this.placeholder = 'Search for Courses / Programs'">

                        <!-- Search Button -->
                        <div class="input-group-prepend">
                            <button class="btn custom-btn" type="button" id="search_button">
                                <i class="ti-search"></i>
                            </button>
                        </div>
                    </div>
            ...
            <div class="row px-4 px-sm-2 px-md-5">
                ...
                <div class="col-12 mb-3 mb-md-4">
                    <h5 class="small_heading text-center">
                        ...
                        <span class="font-weight-bold">"{{ $request->get('query') }}"</span>{{ ' out of ' . count($all_programs) . ' Course(s)/Program(s)' }}<br style="line-break: auto">
                        @if ($all_programs->count() == 0)
                        <span class="subtitle">
                            {{ __('0 Course(s)/Program(s) available') }}</span>
                        @else
                        <span class="subtitle">{{ $total ?? 0 }}
                            {{ __('Course(s)/Program(s) available') }}</span>
                        @endif
                    </h5>
                </div>
                @endif
                ...
                @php
                if($program->has('effective_program_plan')){
                ...
                $course_type = 'Program';
                $price = $program->get('effective_program_plan')[0]['amount'];
                $weeks = round((strtotime($program->get('effective_program_plan')[0]['edate']) - strtotime($program->get('effective_program_plan')[0]['sdate'])) / 604800, 1);
                }else{
                ...
                $chapters = $program->get('parent') ? count($program->get('parent')['chapters']) : count($program->get('chapters'));
                // $classesarr = 
                // $totalclasses = $program->get('parent') ? count($program->get('parent')['classes']) : 0;
                switch ($program->get('type')) {
                case 2:
                $course_type = 'Big Quiz';
                $classes = $chapters.' Chapters';
                break;
                case 3:
                $course_type = 'Classes';
                $classes = $chapters.' Chapters';
                ...
                $price = $program->get('effective_course_plan') ? $program->get('effective_course_plan')[0]['amount'] : $program->get('price') + $program->get('tax');
                $weeks = $program->get('effective_course_plan') ? round((strtotime($program->get('effective_course_plan')[0]['edate']) - strtotime($program->get('effective_course_plan')[0]['sdate'])) / 604800, 1) : '-';
                }
                $subtitle = '';
                @endphp
