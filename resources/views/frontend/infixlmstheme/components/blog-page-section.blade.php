<div class="row pt-5 pb-4 g-0">
    @if (count($blogs)>0)
    <div class="col-12 col-md-7 col-lg-8">
        @foreach ($blogs as $blog)
                    <div class="card rounded-3 ml-xl-5 mb-4 ml-lg-4" style="border-radius: 15px;">
                        <img src="{{ getBlogImage($blog->thumbnail) }}"
                            class="img-fluid custom-img-blog" alt="" >
                        ...
                        <div class="d-flex align-items-center pt-3 gap-2 px-3">
                            ...
                            <span>{{ date('F, d',strtotime($blog->authored_date)) }}</span>
                        ...
                        <div class="card-text px-3 pb-3 pt-2">
                            ...
                            <p>
                                {{ substr(html_entity_decode(strip_tags($blog->description)),0,500) }}
                                @if(strlen(html_entity_decode(strip_tags($blog->description))) > 500) ... @endif
                            </p>
            ...
            <div class="col-12 col-md-5 col-lg-4 pr-4">
                <div class="search mr-xl-5 mr-lg-4 mr-3">
                    ...
                    <form class="search-input-field @if(request()->has('query')) d-flex flex-wrap @endif rounded-3" action="{{ route('blogs') }}" style="margin:auto;">
                        <input type="text" placeholder="Search.." name="query" @if(request()->has('query')) style="flex:0 0 80%" @endif>
                        <button type="submit" @if(request()->has('query')) style="flex:0 0 20%" @endif><i class="fa fa-search"></i></button>
                        @if(request()->has('query'))
                        <a href="{{ route('blogs') }}">Reset</a>
                        @endif
                    </form>
                </div>
