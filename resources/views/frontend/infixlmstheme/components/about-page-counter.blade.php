<div class="">
    <style>
        /* .counter_area::before {
            background-image: url('{{ asset($about->image4) }}');
            margin-right: 26px;
        } */
        /* .counter-image{
            clip-path: polygon(23% 0, 100% 0, 100% 100%, 0 100%);
        } */
        .counter_video {
            height: 490px;
        }

        .counter-image {
            border-radius: 20px;

        }

        /* .center-content-about{
            display: flex;
            align-items: center;
        } */
    </style>
    <section class="aboutPage-sec3">
        <div class="counter_area mt-lg-5 mt-3 px-xl-5">
            <div class="container">
                <div class="row counter_area_row px-3 px-sm-0 px-lg-4">
                    {{-- <div class="col-lg-6 d-flex align-items-center">
                    <div class="counter_wrapper mb-3 mb-lg-0">
                        <div class="single_counter">
                            <h3><span class="">{{ $about->total_teacher }}</span></h3>
                    <div class="counter_content">
                        <h5 class="custom_small_heading">{{ $about->teacher_title }}</h5>
                        <p>{{ $about->teacher_details }}</p>
                    </div>
                </div>
                <div class="single_counter">
                    <h3><span class="">{{ $about->total_student }}</span></h3>
                    <div class="counter_content">
                        <h5 class="custom_small_heading">{{ $about->student_title }}</h5>
                        <p>{{ $about->student_details }}</p>
                    </div>
                </div>
                <div class="single_counter">
                    <h3><span class="">{{ $about->total_courses }}</span></h3>
                    <div class="counter_content">
                        <h5 class="custom_small_heading">{{ $about->student_title }}</h5>
                        <p>{{ $about->student_details }}</p>
                    </div>
                </div>
                <div class="single_counter">
                    <h3><span class="">{{ $about->total_courses }}</span></h3>
                    <div class="counter_content">
                        <h5 class="custom_small_heading">{{ $about->student_title }}</h5>
                        <p>{{ $about->student_details }}</p>
                    </div>
                </div>
            </div>
        </div> --}}
        <div class="col-12 mb-3">
            {{-- <img src="{{ asset('/public/uploads/images/footerimg/ezgif-2-78802b2d5b.mp4') }}" class="w-100 h-100"> --}}
            <div class="counter_video d-block d-lg-none">
                <video class="counter-image h-100 w-100" autoplay loop muted style="object-fit: fill">
                    <source src="{{ asset('/public/uploads/images/footerimg/intro-video.mp4') }}">
                </video>
            </div>
            <div class="counter_video d-none d-lg-block px-xl-5">
                <video class="counter-image h-100 w-100" autoplay loop muted style="object-fit: fill">
                    <source src="{{ asset('/public/uploads/images/footerimg/new-intro-video-com.mp4') }}">
                </video>
            </div>

        </div>
</div>
</div>
</div>
</section>
</div>