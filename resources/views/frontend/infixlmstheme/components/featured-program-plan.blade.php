@if($programplan)
<div class="need_to_learn" @if($featured_programplan_bg != '') style="background-image:url({{asset($featured_programplan_bg)}})" @endif>
    <div class="row justify-content-center px-3 px-lg-5 position-relative" style="z-index: 9">
        <div class="col-md-8 text-center px-xl-5">
            ...
            <p class="text-white mb-3"><span class="text-warning">Price: {{ showPrice($programplan->amount) }}</span></p>
            <a href="{{route('programs.detail',$programplan->programName->id)}}" class="small_btn5 theme_btn py-2 px-4 mt-3">Apply Now</a>
        </div>
    </div>
</div>
@endif
