...
{{-- <div class="breadcrumb_area bradcam_bg_2 hero"> --}}
    {{-- @if ($banner != null) style="background-image: url('{{ asset(@$banner) }}')" @endif> --}}
    ...
    {{-- <div class="col-lg-6">
        <div class="breadcam_wrap">
            ...
            @if ($btntitle != '')
                @if ($btnlink == '' || $btnlink == '#')
                    <button
                        class="font-weight-bold hit ml-1 px-2 px-md-3 py-2 theme_btn mt-md-4 mt-3 {{ $btnclass }}">
                        {{ $btntitle }} </button>
                @else
                    <a href="{{ $btnlink }}"
                        class="font-weight-bold hit ml-1 px-2 px-md-3 py-2 theme_btn mt-md-4 mt-3 {{ $btnclass }}">{{ $btntitle }}</a>
                @endif
            @endif
        </div>
    </div>
 ...
 <div class="hero">
    <div class="hero-inner">
        ...
        <h1> {{ @$title }}</h1>
        <p class="hero-sub">{!! @$sub_title !!}</p>
        <p class="hero-tag">"A struggling student is not a failing student."</p>
    </div>
