                                            ...
                                            <div class="d-flex align-items-center ml-3 pl-20 text-right">
                                                ...
                                                @else
                                                    <a href="#"
                                                        ...
                                                        onclick="goFullScreen({{ $course->id }},{{ $lesson_ids[$current_index - 1] }},{{ request()->program_id ?? 'null' }} ,{{ request()->courseType ?? 'null' }},{{request()->plan_id}})"
                                                        @else
                                    ...
                                    <div class="sumit_skip_btns d-flex align-items-center">
                                        @if (isset($result) && $result['status'] != 'Failed')
                                            <form action="{{ route('lesson.complete') }}" method="post">
                                                ...
                                                @if (request()->has('plan__id'))
                                                <input type="hidden" value="{{ request()->get('plan__id') }}" name="plan_id">
                    ...
                    <script>
                        $(document).ready(function(e) {
                            if ($('#video-id').length) {
                                ...
                                var video = iframe.tagName.toLowerCase() === "iframe" ? iframe.contentDocument.body.getElementsByTagName("video")[0] : iframe;
            ...
            <div class="course__play_warp courseListPlayer">
                ...
                <div class="course__play_list">
                    @php
                        $i = 1;
                    @endphp
                    <div class="theme_according mb-md-3 mb-5" id="accordion1">
            ...
            <div class="card-header pink_bg" id="heading{{ $chapter->id }}">
                <h5 class="mb-0">
                    <button class="btn btn-link text_white collapsed" data-toggle="collapse"
                        ...
                        {{ $chapter->name }} <br>
                        <span class="course_length nowrap">{{ count($chapter->lessons) }} {{ __('frontend.Lectures') }}</span>
                    </button>
                </h5>
            </div>
            <div class="collapse" id="collapse{{ $chapter->id }}" aria-labelledby="heading{{ $chapter->id }}" data-parent="#accordion1">
                <div class="card-body">
                    <div class="curriculam_list">
                        @if (isset($lessons))
                            @foreach ($lessons as $singleLesson)
                                @if ($singleLesson->chapter_id == $chapter->id)
                                    <div class="single_play_list" id="single_lesson_{{ $singleLesson->id }}">
                                        <a class="@if (showPicName(Request::url()) == $singleLesson->id) active @endif" href="#">
                                            @if ($singleLesson->is_quiz == 1)
                                                <div class="course_play_name">
                                                    @php
                                                    $plan_id = request()->plan_id ?? null;
                                                        $programSingleLesson = request()->has('program_id') 
                                                            ? $singleLesson->programCompleted->where('plan_id',$plan_id) 
                                                            : $singleLesson->completed->where('plan_id',$plan_id);
                                                    @endphp
                                                    ...
                                                    @foreach ($programSingleLesson as $lessonItem)
                                                        @if (
                                                            ...
                                                            <label class="primary_checkbox d-flex mb-0">
                                                                <input type="checkbox" {{ $lessonItem && $lessonItem->status == 1 ? 'checked' : '' }} disabled>
                                                    ...
                                                    @foreach ($singleLesson->quiz as $quiz)
                                                        <span class="quizLink"
                                                        @if(request()->has('plan_id'))
                                                            onclick="goFullScreen({{ $course->id }},{{ $singleLesson->id }},{{ request()->program_id ?? 'null' }},{{ request()->courseType ?? 'null' }},{{request()->plan_id}})"
                                                        @else
                                            ...
                                            @else
                                                ...
                                                <div class="course_play_name">
                                                    ...
                                                    @endforeach
                                                    @if(request()->has('plan_id'))
                                                    <span onclick="goFullScreen({{ $course->id }},{{ $singleLesson->id }},{{ request()->program_id ?? 'null' }},{{ request()->courseType ?? 'null' }},{{request()->plan_id}})">
                                                    @else
                                                        <span onclick="goFullScreen({{ $course->id }},{{ $singleLesson->id }},{{ request()->program_id ?? 'null' }},{{ request()->courseType ?? 'null' }})">
                                                        @endif
                                                            {{ $i }}. {{ $singleLesson->name }}
                                                    </span>
                                                </div>
                                                <span class="course_play_duration nowrap">{{ MinuteFormat($singleLesson->duration) }}</span>
                                            @endif
                                        </a>
                                    </div>
                                    @php $i++; @endphp
...
@push('js')
    ...
    @if ($lesson->host == 'XAPI' || $lesson->host == 'XAPI-AwsS3')
        <script>
            @if (!isset($lesson->completed->where('plan_id',$plan_id)->status))
